...
public class IconPacksUI : BaseUI
{
    ...
    {
        Instance = null;
        var enumerable = Packs.Where(x => !x.FromMainRepo);
        GeneralUtils.SaveText("OtherPacks.json", JsonConvert.SerializeObject(enumerable, enumerable.GetType(), Formatting.Indented, new()), path: Path);
    }
