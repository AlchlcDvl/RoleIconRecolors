...
namespace FancyUI.Patches
{
    ...
    {
        [HarmonyPostfix]
        public static void Postfix(PlayerIsWhisperingDecoder __instance, ref string __result, ChatLogMessage chatLogMessage, MentionPanel mentionPanel, GamePhase chatPhase, UIController uiController, List<HudChatStyle> chatStyles, ChatWindowType chatWindowType)
        {
            ...
            var isSenderOrReceiver = myPosition == __instance.whispererId || myPosition == __instance.whispereeId;
            var isWildling = Pepper.GetMyRole() == Role.WILDLING;
            var isSerialKiller = Pepper.GetMyRole() == Role.SERIALKILLER && Constants.IsBTOS2();

            if (!isSenderOrReceiver && (isWildling || isSerialKiller))
            {
                string xToY;
                if (isWildling)
                {
    ...
    {
        [HarmonyPostfix]
        public static void Postfix(PlayerWhisperDecoder __instance, ref string __result, ChatLogMessage chatLogMessage, MentionPanel mentionPanel, GamePhase chatPhase, UIController uiController, List<HudChatStyle> chatStyles, ChatWindowType chatWindowType)
        {
            var whisperId = __instance.whispererId == Pepper.GetMyPosition() || __instance.whispereeId == Pepper.GetMyPosition();
            if (whisperId)
            {
