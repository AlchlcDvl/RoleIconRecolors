...
{
    public static void Prefix() => PatchRoleCardIcon.Index = 0;
}
...
public static class PatchRoleCardAbilityIcons1
{
    public static void Postfix(RoleCardPanel __instance)
    {
        ...
        if (sprite != AssetManager.Blank && __instance.roleInfoButtons[PatchRoleCardIcon.Index] != null)
        {
            __instance.roleInfoButtons[PatchRoleCardIcon.Index].abilityIcon.sprite = sprite;
            PatchRoleCardIcon.Index++;
        }
    }
}
...
public class FixRoleChanges
{
    public static void Postfix(RoleCardPanelBackground __instance, ref Role role)
    {
        ...
        if (ability1 != AssetManager.Blank && panel.roleInfoButtons[PatchRoleCardIcon.Index] != null)
        {
            panel.roleInfoButtons[PatchRoleCardIcon.Index].abilityIcon.sprite = ability1;
            PatchRoleCardIcon.Index++;
        }

        var ability2 = AssetManager.GetSprite($"{name}_Ability_2");

        if (ability2 != AssetManager.Blank && panel.roleInfoButtons[PatchRoleCardIcon.Index] != null)
        {
            panel.roleInfoButtons[PatchRoleCardIcon.Index].abilityIcon.sprite = ability2;
            PatchRoleCardIcon.Index++;
        }

        var attribute = AssetManager.GetSprite($"Attributes_{Utils.FactionName(Pepper.GetMyFaction(), role)}");

        if (attribute != AssetManager.Blank && panel.roleInfoButtons[PatchRoleCardIcon.Index] != null)
        {
            panel.roleInfoButtons[PatchRoleCardIcon.Index].abilityIcon.sprite = attribute;
            PatchRoleCardIcon.Index++;
        }

        var nommy = AssetManager.GetSprite("Necronomicon");

        if (nommy != AssetManager.Blank && Service.Game.Sim.info.roleCardObservation.Data.powerUp == POWER_UP_TYPE.NECRONOMICON && panel.roleInfoButtons[PatchRoleCardIcon.Index] != null)
        {
            panel.roleInfoButtons[PatchRoleCardIcon.Index].abilityIcon.sprite = nommy;
            PatchRoleCardIcon.Index++;
        }
    }
}